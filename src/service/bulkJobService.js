...
async function createJob(req, response) {
  ...
  const errCode =
    ...
    "_" +
    bulkJobRequestMessages.CREATE.EXCEPTION_CODE;
  if (
    !data.request ||
    !data.request.process_id ||
    !data.request.program_id ||
    !data.request.type ||
    !data.request.createdby
  ) {
  ...
  try {
    ...
    rspObj.errMsg = sequelizeErrorMessage
      ? sequelizeErrorMessage.message
      : error.message || bulkJobRequestMessages.CREATE.FAILED_MESSAGE;
    rspObj.responseCode = responseCode.SERVER_ERROR;
    loggerError(rspObj, errCode + errorCode.CODE2);
    loggerService.exitLog({ responseCode: rspObj.responseCode }, logObject);
    return response
      .status(500)
      .send(errorResponse(rspObj, errCode + errorCode.CODE2));
  }
}
...
async function readJob(req, response) {
  ...
  const errCode =
    programMessages.EXCEPTION_CODE +
    "_" +
    bulkJobRequestMessages.READ.EXCEPTION_CODE;
  try {
    const readResponse = await model.bulk_job_request.findOne({
...
async function updateJob(req, response) {
  ...
  const errCode =
    programMessages.EXCEPTION_CODE +
    "_" +
    bulkJobRequestMessages.UPDATE.EXCEPTION_CODE;
  ...
  try {
    const updateResponse = await model.bulk_job_request.update(
      updateValue,
      updateStatement
    );
    ...
    rspObj.errMsg =
      error.message || bulkJobRequestMessages.UPDATE.UPDATE_FAILED_MESSAGE;
    rspObj.responseCode = responseCode.SERVER_ERROR;
    loggerError(rspObj, errCode + errorCode.CODE3);
    loggerService.exitLog({ responseCode: rspObj.responseCode }, logObject);
    return response
      .status(500)
      .send(errorResponse(rspObj, errCode + errorCode.CODE3));
  }
}
...
async function searchJob(req, response) {
  ...
  try {
    ...
    rspObj.errMsg =
      error.message || bulkJobRequestMessages.SEARCH.FAILED_MESSAGE;
    rspObj.responseCode = responseCode.SERVER_ERROR;
    loggerError(rspObj, errCode + errorCode.CODE2);
    loggerService.exitLog({ responseCode: rspObj.responseCode }, logObject);
    return response
...
function loggerError(data, errCode) {
  ...
  errObj.edata = {
    ...
    stacktrace: _.truncate(JSON.stringify(data), {
      length: stackTrace_MaxLimit,
    }),
  };
