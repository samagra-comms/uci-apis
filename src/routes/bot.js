...
async function getByParam(req, res) {
    if (req.query.name) {
        ...
        if (bot instanceof Bot) {
            // Add logic
            const logic = [];
            for (let i = 0; i < bot.logic.length; i++) {
                logic.push(await ConversationLogic.query().findById(bot.logic[i]));
            }
            bot.logic = logic;
            res.send({data: bot});
        } else
    ...
    if (!isExisting) {
        ...
        try {
            ...
            if (isValidUserSegment && isValidCL) {
                const inserted = await Bot.query(trx).patch(data).findById(req.params.id);
                await trx.commit();
