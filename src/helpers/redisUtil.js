...
class RedisManager {
  constructor() {
    ...
    this.redisClient.on("ready", function () {
      logger.info({
        msg: `✅ Redis Server connected to [${envVariables.DOCK_REDIS_HOST}:${envVariables.DOCK_REDIS_PORT}]`,
      });
    });
  }
  ...
  isConnected() {
    this.msgredisClient.on("connect", function () {
      logger.info({
        msg: `✅ Redis Server connecting to [${envVariables.DOCK_REDIS_HOST}:${envVariables.DOCK_REDIS_PORT}]`,
      });
    });
  }
  ...
  isReconnecting() {
    this.redisClient.on("reconnecting", function () {
      logger.info({
        msg: `❌ Redis Server reconnecting to [${envVariables.DOCK_REDIS_HOST}:${envVariables.DOCK_REDIS_PORT}]`,
      });
