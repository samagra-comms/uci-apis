...
async function checkInbound(req, res) {
  let checks = [];
  fetch(`${process.env.INBOUND_URL}/health`)
    .then(async (s) => {
      ...
      response.sendHealthSuccessRes(
        ...
        checks,
        res,
        HealthMessages.INBOUND.SUCCESS_CODE
      );
      } else{
        response.sendHealthErrorRes(
          req,
          res,
          HealthMessages.INBOUND.FAILED_CODE,
          errorCode,
          HealthMessages.INBOUND.FAILED_MESSAGE,
          false,
          errCode
        );
      }
